...
[`260710-ahp-graph-entropy.pdf`](https://github.com/hamlett-neil-ur/ahp-graph-entropy/blob/main/research-paper/260711-ahp-graph-entropy.pdf).

## 𝓣𝓸 𝓓𝓸. 🏗️🧱👷

### Back out the  emphasis directed-acyclic graphs (DAGs) from narrative.

We use strictly-acyclic directed graphs within the narrow context of checking for violations of von Neumann/Morgenstern transitivity in comparative judgements. By framing AHP in terms of Perron-Frobenius theory, Saaty implicitly interpretes comparative judgements as an exercise in imputing Markovian state transitions. From an indifference-principle perspective — which Saaty explicitly did not apply — the off-diagonal elements of an AHP reciprocal matrix resemble state-transition odds. Strict imposition of acyclicity leads to Markovian degeneracy, in which the terminal distributions are concentrated in "sink" vertices of the graph. What's more, the seminal tournament-graph inspiration ([`Brown, et al (2020)`](https://doi.org/10.1016/j.laa.2019.09.026)) reflects a misconception that tournament graphs are acyclic.

Mechanically, we use complete directed graphs with two edges — one in each direction — between each vertex pair. The weights assigned to these "mirrored" parallel edges correspond to the off-diagonal values of the AHP diagonal matrix. Such directed-graph constructs make convenient the mechanics of calculating transition-probability matrices ([`Chung (2005)`](https://doi.org/10.1007/s00026-005-0237-z)) and normalized graph-Laplacians according to [`Bauer's (2012)`](http://dx.doi.org/10.1016/j.laa.2012.01.020) approach. We consequently find three viable approaches for construction of Laplican matrices on which graph spectra are commonly based.

1. *k*-generalized Hermitian adjacency matrices described by [`Yu, et al (2023)`](https://doi.org/10.1016/j.disc.2022.113254) and [`Wang, et al (2024)`](http://dx.doi.org/10.61091/um119-07). 
2. [`Chung's (2005)`](https://doi.org/10.1007/s00026-005-0237-z) transition-probability approach based on edge weights.
3. [`Bauer's (2012)`](http://dx.doi.org/10.1016/j.laa.2012.01.020) normalized graph-Laplacian operator, which might in special cases produce results equivalent to [`Chung (2005)`](https://doi.org/10.1007/s00026-005-0237-z). 

Alternatively, [`Chen, et al (2015)`](https://doi.org/10.3390/e17063710) and [`Fung (2025)`](https://doi.org/10.1007/s00026-005-0237-z) describe a graph-entropy approach — our ultimate objective — that neglects graph-connectivity information considered in the other approaches. Although this avoids Markovian degeneracy associated with strict DAGs, the ommitted vertex adjacency seems to contain something essential to graph analysis. Reinterpreting comparative judgements as complete, directed graphs with mirrored parallel edges circumvents the DAG-degeneracy problem, anyway.  Consequently, the [`Chen, et al (2015)`](https://doi.org/10.3390/e17063710) and [`Fung (2025)`](https://doi.org/10.1007/s00026-005-0237-z) approaches are not preferred here.

***General observation***. Many of these graph-algebra approaches seem to be *ad-hoc*. That is, they do not begin from fundamental axioms, like set theory and decision science. *Perron-Frobenius* theory ([`Horn & Johnson (2012)`](https://doi.org/10.1017/CBO9780511810817); [`Hawkins (2021)`](https://doi.org/10.1007/978-3-030-59242-4)) provides the foundational framework from which many of these *ad-hoc* methods arise. Relating AHP comparative judgements to Markovian-state transitions appears to represent Saaty's essential insight. Circumspectful subsequent exploration therefore must tie methodologies applied herein to the Perron-Frobenius Theorem. 
...
* With AHP (and MAUT) we begin with a set of decision elements ℌ⊇{𝑒₁,⋯,𝑒<sub>𝑁</sub>} for hierarchy model ℌ. Moreover we have a set of options 𝒪⊇{ℴ₁,⋯,𝑜<sub>𝑀</sub>} from which to select. We also have a matrix of measurements  𝑹<sub>ℌ</sub>=[𝒓₁ ⋯ 𝒓<sub>𝑁</sub>], within which each column 𝒓<sub>𝑛</sub> is a vector of measurements for each ℴ<sub>𝑚</sub>∈𝒪 with respect to the 𝑛<sup>th</sup> element in ℌ. We standardize 𝑹<sub>ℌ</sub> to obtain 𝑷<sub>ℌ</sub>, and as in (12b) of [`260710-ahp-graph-entropy.pdf`](https://github.com/hamlett-neil-ur/ahp-graph-entropy/blob/main/research-paper/260711-ahp-graph-entropy.pdf), our utility-optimization operation becomes 𝒑<sub>target</sub>=(𝑷<sub>ℌ</sub>⊗𝜴<sub>ℌ</sub>)𝝎<sub>target</sub>.
